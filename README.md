...
**v2.0.3**에서는 **ESC 키 긴급 정지**, **오버레이 레이아웃 초기화**, **향상된 사용성**이 추가되어 더욱 편리해졌습니다.
...
- **레이아웃 초기화** 기능으로 오버레이가 화면 밖으로 사라졌을 때 즉시 복구할 수 있습니다. **[NEW in 2.0.3]**

### 3. 🚨 ESC 키 긴급 정지 (Emergency Stop) **[NEW in 2.0.3]**
...
1. [Releases](https://github.com/your-repo/d4-helper/releases) 페이지에서 `D4Helper_v2.0.3.exe` 다운로드
2. 실행 파일을 더블클릭하여 실행
3. 프로그램이 자동으로 시작됩니다
...
- ✅ **오버레이 드래그 수정**: 오버레이 창이 드래그되지 않던 문제를 완벽하게 수정했습니다. (화면 좌표 기반 이동 로직 적용)
- ✅ **설치 파일 지원**: 이제 `setup.exe` 형태의 설치 파일을 제공하여 더욱 쉽게 설치할 수 있습니다.
- ✅ **문서 최신화**: 최신 UI 스크린샷과 버전 정보가 반영되었습니다.
